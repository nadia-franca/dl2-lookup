...
JIRA_TOKEN = st.secrets.get("JIRA_TOKEN", "ATATT3xFfGF0ZZUU7hzdRgrYNZQQ4xsb4uY-xr7O1UzWFJk5WJn3QeNTpOsXZXOB2L8MGSGZiBHo78OXObcT8asX76sacZjb1iIcoR1H6MvIV52vIlDaxm26sRLxu_-pdfu3jj4wAMXlZjSWpZna6l4MpMf7xHDFaTGpU9_mkYyAxwQEmH31Bmc=14543160")
...
def analyze(data, key):
    ...
    if cid:
        mb_links += [
            ("CX Eligibility V4", f"https://metabase.us.gympass.cloud/question/44902-cx-eligibility-v4?client_id={cid}", "Check employee eligibility status"),
            ("Client Profile — Darwin", f"https://darwin.wellhub.com/clients/{cid}/profile", "Open client profile"),
            ("Staff Users", f"https://darwin.wellhub.com/clients/{cid}/staff_users", "Check admin roles"),
            ("Subscription", f"https://darwin.wellhub.com/clients/{cid}/subscription", "Check subscription status"),
        ]
    if email:
        mb_links.append(("Notification Events", f"https://metabase.us.gympass.cloud/question/44903?email={urllib.parse.quote(email)}", "Check emails sent to this address"))
    ...
    cat_links = {
        ...
        "EF Process / Upload": [
            ...
            ("EF Check Updated and Created", "https://metabase.us.gympass.cloud/question/44900", "See upload history"),
        ],
        "Report / Data access": [
            ("WH Details / Seat Usage", "https://metabase.us.gympass.cloud/question/44905", "Filter by client_id and billing period"),
            ("Deduction File Helper", "https://metabase.us.gympass.cloud/question/44906", "For invoice reconciliation"),
        ],
    }
    for label, url, hint in cat_links.get(category, []):
        if cid: url += f"?client_id={cid}"
        mb_links.append((label, url, hint))
